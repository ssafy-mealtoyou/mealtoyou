...
public class MessageProcessingAdaptor {
    ...
    public Mono<RecentMessage> getRecentChats(String groupId) {
        Pageable pageable = PageRequest.of(0, 3, Sort.by(Sort.Direction.ASC, "timestamp"));
        // 최근 3개의 채팅 메시지 조회
        return chatRepository.findTop3ByGroupIdOrderByTimestampDesc(Long.parseLong(groupId), pageable)
                ...
                .map(chats -> {
                    ...
                    Message message1 = !chats.isEmpty() ? chats.get(0).getMessage() : null;
                    Message message2 = chats.size() >= 2 ? chats.get(1).getMessage() : null;
                    Message message3 = chats.size() >= 3 ? chats.get(2).getMessage() : null;
