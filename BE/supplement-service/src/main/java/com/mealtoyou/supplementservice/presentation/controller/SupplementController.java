...
public class SupplementController {
	...
	public Mono<ResponseEntity<String>> deleteSupplement(@RequestHeader("Authorization") String token, @PathVariable("supplement_id") Long id) {
		return supplementService.deleteSupplement(token,id)
			.map(response -> ResponseEntity.ok().body(response))
			.defaultIfEmpty(ResponseEntity.notFound().build())
			.onErrorResume(e -> Mono.just(ResponseEntity.badRequest().body("Error deleting supplement: " + e.getMessage())));
	}
}
