...
//        Mono<User> userMono = userRepository.findByEmail(oAuth2UserInfo.email())
//                .flatMap(Mono::just) // 기존 사용자를 그대로 반환
//                .switchIfEmpty(Mono.fromSupplier(oAuth2UserInfo::toEntity)); // 새로운 사용자 생성
//        return userMono.flatMap(userRepository::save).block(); // userRepository.save()에 User 객체 전달
//    }
//
//}
